...
from build_model import baseline_model
...
encode = LabelEncoder()
dataset = pd.read_csv(f"{BASE_DIR}\Original_Dataset.csv")
y2 = dataset["Disease"]
...
with open(f"{BASE_DIR}/logisticregression33_model.pkl", "rb") as f:
    model = pickle.load(f)
with open(f"{BASE_DIR}/KNN_model.pkl", "rb") as f:
    KNN_model = pickle.load(f)
with open(f"{BASE_DIR}/DecisionTree_model.pkl", "rb") as f:
    DecisionTree_model = pickle.load(f)
with open(f"{BASE_DIR}/RandomForest_model.pkl", "rb") as f:
    RandomForest_model = pickle.load(f)

with open(f"{BASE_DIR}/SVM_model.pkl", "rb") as f:
    SVM_model = pickle.load(f)
ANNs = models.load_model(f"{BASE_DIR}/keras_model.keras")

with open(f"{BASE_DIR}/sklearn_pipeline2.pkl", "rb") as f:
...
def predict_pipeline(symptoms):
    ...
    result = get_disease_name(y_pred[0]).strip("Disease_")
    return result
